...
| `CAO_MAX_WAVE_IN_FLIGHT` | `3` | int | ADT-6: per-supervisor cap on concurrent `assign`/`handoff` children (shared budget). Excess requests are FIFO-queued server-side (`status=queued` side table — not a live terminal). Assign holds a slot until `delete_terminal`; handoff until the call completes (wave slot releases even if a diagnostic terminal is preserved — the global terminal cap still counts that terminal). Queue drain never overruns `CAO_MAX_ACTIVE_TERMINALS`. |
| `CAO_WAVE_WAIT_TIMEOUT_MAX` | `300` (ceiling `900`) | int | ADT-6: max seconds for `GET /wave/wait/{queue_id}` long-poll (no busy poll). |
| `CAO_WAVE_UNBOUND_LEASE_S` | `30` | float | ADT-6: monotonic lease for unbound in-flight reservations (admit→bind). Bound terminals never expire. |
| `CAO_WAVE_HANDOFF_QUEUE_LEASE_S` | `60` | float | ADT-6: queued handoff ownership lease covering enqueue→wait TOCTOU; active waiters refresh/hold. |
| `CAO_WORKSPACE_BACKEND` | `shared` | str | D11 / T1.4: workspace backend (`shared` \| `worktree` \| `rift` \| `auto`). Precedence: explicit assign/handoff/HTTP `workspace=` > this env > `shared`. `auto` probes rift → worktree → loud shared fallback. See [workspace-backends.md](workspace-backends.md). |
